...
const handleLogout = async() =>{
    ...
    const  SidebarSection=({
        ...
        return (
            <div className='mb-4'>
             <h3 className='mb-2 pz-2 text -sm font-medium text-muted-foreground'> {title}</h3> 
             <div className='space-y-1 w-[260px] '>
                {chats.map((chat) => (
                    <Link
                        ...
                        href={`/chat/${chat._id}`}
                        className={cn(
                            "flex justify-around items-center px-2 py-1 rounded-1g  group hover:bg-[#dae3ef]",
                            pathname === `/chat/${chat._id} && bg-[#dae3ef]` 
                        )}>
<span className='truncate text-gray-700 text-xs flex-1'> {chat.title}</span>
<Button type='button'  variant='ghost' size='icon' className='opacity-0 group-hover:opacity-100 h-8 w-8 ' onClick={(e)=>handleDeleteChat(chat?._id,e)} > 
    <Trash2 className='h-4 w-4 text-gray-700 text-xs flex-1' />
</Button>
    ...
    const  SidebarContent=()=>(
        <div className='flex h-full flex-col bg-[#f7fcff]'>
            <div className='sticky  top-0 z-10 flex items-center justify-between border-b bg-[#f7fcff] p-4'>
                <Link href='/'>
                <img src="/images/deepseek-title.svg" alt="deepseek-title" className='h-20 2-40 text-gray-300' />
                </Link>
            ...
            <div className='p-4'>
                <Button variant='outline' disabled={isChatLoading}  onClick={handleCreateChat} className='justify-start gap-2 bg-[#dce9ff] px-6 py-6 rounded-xl text-blue-500 hover :bg-blue-100'>
             ...
             <ScrollArea  className='flex-1 px-2' >
                {
                    ...
                    (
                        ...
                        <>
                        <SidebarSection title='today' chats ={todayChats} pathname={pathname}/>
                        <SidebarSection title='Yesterday' chats ={yesterdayChats} pathname={pathname}/>
                        <SidebarSection title='30 Days' chats ={last30DaysChats} pathname={pathname}/>
                        {
                            olderChats.length>0 && (
                                <SidebarSection title='Older' chats ={olderChats} pathname={pathname}/>
